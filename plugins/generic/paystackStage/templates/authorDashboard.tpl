		...
		<tabs :track-history="true">
			<tab id="workflow" label="{translate key="manager.workflow"}">
				...
				{foreach from=$workflowStages item=stage}
					...
					{/if}
					{if $stage.path == 'editorial' && $submission->getStageId() >= $stage.id}
						{assign var=selectedTabIndex value=$selectedTabIndex+1}
					{/if}
				{/foreach}
				...
				<div id="stageTabs" class="pkp_controllers_tab">
						...
						{foreach from=$workflowStages item=stage}
							{if $stage.path == 'editorial'}
								<li class="pkp_workflow_paystack stageIdPayment">
									<a class="paystack" href="#paystackPaymentPanel">{translate key="plugins.paymethod.paystack.workflow.tab"}</a>
								</li>
							{/if}
					...
					<div id="paystackPaymentPanel" class="paystack-workflow-panel">
						<div class="paystack-workflow">
							...
							<div class="paystack-workflow__status paystack-workflow__status--{$paystackFeeStatus|default:'due'|escape}">
								<p>
